...
public interface MotionRepository extends JpaRepository<Motion, SensorDataPK> {
	 ...
	 */
	@Query(
		value = "SELECT sensor_id, data, timestamp FROM motion WHERE sensor_id = ?1 ORDER BY timestamp LIMIT 1",
		nativeQuery = true
	)
	 ...
	 */
	@Query(
		value = "SELECT sensor_id, data, timestamp FROM motion WHERE (sensor_id, timestamp) IN (select sensor_id, MAX(timestamp) FROM motion GROUP BY sensor_id)",
		nativeQuery = true
	)
