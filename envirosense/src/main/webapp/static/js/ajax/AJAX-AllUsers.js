...
function runAjax() {
	...
	xmlHttp.open("GET", "http://localhost:8080/envirosense/getuser/", true);
	xmlHttp.send();
}
...
function readyStateChange(xmlHttp) {
	...
	if (xmlHttp.status == 200) {
		...
		if (xmlHttp.readyState == 4) {
			...
			/*
			 * The instantiation of the location on where the JSON object's data
			 * will go to.
			 */
			...
			for (x in dataList) {
				/*
				 * The algorithm to parse the JSON object and display it to the
				 * user.
				 */
