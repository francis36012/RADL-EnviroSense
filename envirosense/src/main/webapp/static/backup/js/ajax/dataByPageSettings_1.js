...
function loadDataByPageSettings(jsonObject, domElement) {
	...
	var roomCategory = createNode("h3", null, null);
	...
	for (var index = 0; index < jsonObject.length; index++) {
		...
		if (entryList[jsonElement.sensorType] === undefined) {
			...
			entryList[jsonElement.sensorType] = sensorEntry;
		}
	};
	
	var allCategory = Object.keys(entryList);
	for (var index = 0; index < allCategory.length; index++) {
		listContentsContainer.appendChild(entryList[allCategory[index]]);
	}
